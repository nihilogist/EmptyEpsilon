...
REGISTER_SCRIPT_CLASS(ShipTemplate)
{
    ...
    REGISTER_SCRIPT_CLASS_FUNCTION(ShipTemplate, setTubeTurretRotationSpeed);
...
void ShipTemplate::setTubeBatterySize(int index, int batterySize) {
    if (index < 0 || index >= max_weapon_tubes)
        return;
...
void ShipTemplate::setTubeTurretArc(int index, float turretArc) {
    if (index < 0 || index >= max_weapon_tubes)
        return;
    weapon_tube[index].turretArc = turretArc;
}

void ShipTemplate::setTubeTurretRotationSpeed(int index, float turretRotationSpeed) {
    if (index < 0 || index >= max_weapon_tubes)
        return;
    weapon_tube[index].turretRotationSpeed = turretRotationSpeed;
}
...
string getSystemName(ESystem system)
{
    switch(system)
    {
    case SYS_Reactor: return "Plasma Reactor";
    case SYS_BeamWeapons: return "Lance Batteries";
    case SYS_MissileSystem: return "Main Batteries";
    case SYS_Maneuver: return "Thrusters";
    case SYS_Impulse: return "Plasma Drives";
    case SYS_Warp: return "Warp Drive";
    case SYS_JumpDrive: return "Jump Drive";
    case SYS_FrontShield: return "Void Shield Generators";
