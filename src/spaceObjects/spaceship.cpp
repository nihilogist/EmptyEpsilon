...
void SpaceShip::drawOnRadar(sf::RenderTarget& window, sf::Vector2f position, float scale, bool long_range)
{
    ...
    if (!long_range && (!my_spaceship || (getScannedStateFor(my_spaceship) == SS_FullScan)))
    {
        ...
        for(int n = 0; n < max_beam_weapons; n++)
        {
            ...
            // TODO: Make this color configurable.
            a[0].color = sf::Color(color.r, color.g, color.b, color.a / 1.5);
            a[1].color = sf::Color(color.r, color.g, color.b, color.a / 1.5);
            ...
            {
                turret_line[i].color = sf::Color(color.r, color.g, color.b, color.a / 1.5);
...
{
    // Complete rewrite of damage system
    // If the game is configured to handle system damage:
