...
: GuiElement(owner, "SHIP_DESTROYED_POPUP"), owner(owner)
{
    ...
    (new GuiLabel(ship_destroyed_overlay, "SHIP_DESTROYED_TEXT", "SYSTEMS OFFLINE", 70))->setPosition(0, 0, ACenter)->setSize(500, 100);
    (new GuiButton(ship_destroyed_overlay, "SHIP_DESTROYED_BUTTON", "Proceed (with ref)", [this]() {
        this->owner->destroy();
